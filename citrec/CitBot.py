...
def flip_page_rec(value, time):
    ...
    try:
        ...
        return {
            "text": "This is an outdated message (more then 60 minutes), please send me the citation context again 🥺"
        }
...
def flip_page_ref(value, time):
    ...
    try:
        ...
        "blocks": render_template(
            ...
            ref_list_id=ref_list["_id"],
            next_page=True if len(ref_list[(page * 5):]) > 5 else False,
            page=page,
        ),
        "updateBlock": "true",
        "ts": time,
    ...
    try:
        ...
        for paper in (rec_or_ref_result["papers"])[(page * 5):]:
            if paper["_id"] == paper_id or paper["_id"] == ObjectId(paper_id):
                paper["inList"] = True
        results.update_one({"_id": ObjectId(ind)}, {"$set": {'papers': rec_or_ref_result["papers"]}})
        if rec_or_ref == "rec":
            return {
        "blocks": render_template(
            "rec_result.json.jinja2",
        ...
        "blocks": render_template(
            ...
            next_page=True if len(rec_or_ref_result["papers"][(page * 5):]) > 5 else False,
            page=page,
        ),
        "updateBlock": "true",
        "ts": time,
        ...
        return {
            "text": "I have add this paper to the marking list, send \"!list\" to see the marking list 😉\n However, this message is outdated, so I could not update the message 🥺"
        }
...
def find_papers_in_list(user_id):
    ...
    for i, source in ids_sources:
        if source == "dblp":
            ...
            finded_papers.append(dic)
    list_id = ObjectId()
    lists_temp.insert_one({"_id": list_id, "papers": finded_papers, "expireAt": datetime.datetime.utcnow()
                + datetime.timedelta(minutes=20),})
    return list_id, finded_papers

def del_paper_in_list(value, time, user_id):
    list_id, paper_id, paper_source = tuple(value.split(","))
    ...
    try:
        ...
        list_papers = lists_temp.find({"_id": ObjectId(list_id)}).next()["papers"]
        for paper in list_papers:
            if paper["_id"] == paper_id or paper["_id"] == ObjectId(paper_id):
                list_papers.remove(paper)
                break
        lists_temp.update({"_id": ObjectId(list_id)}, {"$set": {'papers': list_papers}})
        return {
        "blocks": render_template("mark_list.json.jinja2", list_id=list_id, marked_papers=list_papers),
        "updateBlock": "true",
        "ts": time,
    }
    # no temp, find papers in list again
    except (StopIteration, ValueError):
        list_id, list_papers = find_papers_in_list(user_id)
        return {
        "blocks": render_template("mark_list.json.jinja2", list_id=list_id, marked_papers=list_papers),
        "updateBlock": "true",
        "ts": time,
